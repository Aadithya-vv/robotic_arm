...
class FrameProcessingService:
    ...
    def run(self):
        ...
        self.workspace.web_clusters = {"renamed":{},"ignored":[],"deleted":[],"accepted":[],"selected":[],"expanded":[],"generated":[],"clustering":{"state":"idle","progress":0,"error":None}}
        ...
        def finished(index, elapsed_ms):
            ...
            labels=[{"class_name":x.properties.get("ai_class") or "Object","confidence":x.confidence,"object_id":x.candidate_id,"x":x.region.x,"y":x.region.y,"width":x.region.width,"height":x.region.height,"embedding":[number for feature in x.features for number in feature.values]} for x in result[1].objects] if result else []
        ...
        def done(completed,error):
            failures = int(state["metrics"].get("failed", 0))
            final_state = "partial" if completed and failures else "complete" if completed else "failed" if error else "cancelled"
            state.update(state=final_state,phase="complete" if completed else "stopped",current=total if completed else state["current"],frame=total if completed else state["frame"],eta=0.0,error=error)
