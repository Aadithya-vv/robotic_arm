# ENG-012 — Knowledge Engine Specification
...
Represent reusable semantic knowledge. This specification refines the catalogue responsibility into an implementation-reviewable behavioral boundary without selecting algorithms, concrete APIs, or concrete dependencies.

## 2. Scope

The Engine accepts its approved architectural inputs, validates them, performs the responsibilities owned by ENG-012, returns explicit outcomes, and exposes sufficient lifecycle and diagnostic information for deterministic coordination and review.
...
**Primary input:** Semantic concepts, relationships, queries, and validated Knowledge Update Requests from ENG-022.
...
**Primary output:** Reusable knowledge state, reasoning context, and explicit applied/rejected Knowledge Update Request outcomes.
...
**Approved contract relationships:** Semantic Inventory context; Knowledge Update Requests from ENG-022; Configuration and Logging. ENG-012 alone validates and applies knowledge updates.
...
This Engine does not own scene tracking, affordance decisions, planning, feedback capture, or runtime temporary memory. It shall not bypass another architectural layer, duplicate another Engine's decision authority, or absorb convenience behavior outside its catalogue responsibility.
...
**Draft state model:** Empty>Loading>Available; Available>Querying>Available; Available>ValidatingUpdate>ApplyingUpdate>Available|UpdateRejected; active states may enter Invalid|Closed.
...
| Engine purpose/responsibilities | ABP-04 ENG-012 |
