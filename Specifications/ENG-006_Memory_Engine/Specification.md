# ENG-006 — Memory Engine Specification
...
Manage runtime memory ownership. This specification refines the catalogue responsibility into an implementation-reviewable behavioral boundary without selecting algorithms, concrete APIs, or concrete dependencies.

## 2. Scope

The Engine accepts its approved architectural inputs, validates them, performs the responsibilities owned by ENG-006, returns explicit outcomes, and exposes sufficient lifecycle and diagnostic information for deterministic coordination and review.
...
**Primary input:** Validated runtime value, owner identity, and context request.
...
**Primary output:** Managed temporary state result or cleanup outcome.
...
**Contract relationships for engineering:** Configuration and Logging contracts; lifecycle coordination contract.
...
This Engine does not own persistent semantic knowledge, execution history, or configuration storage. It shall not bypass another architectural layer, duplicate another Engine's decision authority, or absorb convenience behavior outside its catalogue responsibility.
...
**Draft state model:** Created>Ready; Ready>Active>Ready; Ready>Cleaning>Ready; Ready>Disposed.
...
| Engine purpose/responsibilities | ABP-04 ENG-006 |
