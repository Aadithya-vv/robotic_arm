# ENG-003 — Configuration Engine Specification
...
| Engine | ENG-003 — Configuration Engine |
...
Manage system configuration. This specification refines the catalogue responsibility into an implementation-reviewable behavioral boundary without selecting algorithms, concrete APIs, or concrete dependencies.

## 2. Scope

The Engine accepts its approved architectural inputs, validates them, performs the responsibilities owned by ENG-003, returns explicit outcomes, and exposes sufficient lifecycle and diagnostic information for deterministic coordination and review.
...
**Primary input:** Configuration source and settings request.
...
**Primary output:** Validated immutable runtime settings or validation failure.
...
**Contract relationships for engineering:** External local configuration source; Logging contract.
...
This Engine does not own application startup, dependency resolution, or consumer-specific policy. It shall not bypass another architectural layer, duplicate another Engine's decision authority, or absorb convenience behavior outside its catalogue responsibility.
...
**Draft state model:** Unloaded>Loading>Validating>Available|Invalid; Available>Reloading>Validating.
...
| Engine purpose/responsibilities | ABP-04 ENG-003 |
