# ENG-017 — Execution Engine Specification
...
| Engine | ENG-017 — Execution Engine |
...
Execute validated TaskIR. This specification refines the catalogue responsibility into an implementation-reviewable behavioral boundary without selecting algorithms, concrete APIs, or concrete dependencies.

## 2. Scope

The Engine accepts its approved architectural inputs, validates them, performs the responsibilities owned by ENG-017, returns explicit outcomes, and exposes sufficient lifecycle and diagnostic information for deterministic coordination and review.
...
**Primary input:** Validated TaskIR under Contracts/TaskIRContract.md and a matching Approval Token issued by ENG-020.
...
**Primary output:** Execution state, progress events, and terminal outcome.
...
- A valid Approval Token issued by ENG-020 is present and is bound to the exact TaskIR identity, version, and correlation context.
...
**Approved contract relationships:** ENG-014 provides validated TaskIR; ENG-020 provides the matching Approval Token; ENG-018 provides simulation communication; Event Bus and Logging provide platform services.
...
This Engine does not own task planning, TaskIR generation, user approval, Approval Token creation/refresh, simulation transport implementation, or replay storage. It shall not bypass another architectural layer, duplicate another Engine's decision authority, or absorb convenience behavior outside its catalogue responsibility.
...
**Draft state model:** Idle>ValidatingTaskIR>ValidatingApproval>Scheduled>Running>Completed|Failed|Cancelled; Running>Paused>Running where approved.
...
| Engine purpose/responsibilities | ABP-04 ENG-017 |
