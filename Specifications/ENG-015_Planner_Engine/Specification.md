# ENG-015 — Planner Engine Specification
...
Own task-intent understanding and generate Semantic Plans. This specification refines the catalogue responsibility into an implementation-reviewable behavioral boundary without selecting algorithms, concrete APIs, or concrete dependencies.

## 2. Scope

The Engine accepts its approved architectural inputs, validates them, performs the responsibilities owned by ENG-015, returns explicit outcomes, and exposes sufficient lifecycle and diagnostic information for deterministic coordination and review.
...
**Primary input:** Validated demonstration data from ENG-021, goal context, semantic understanding, and affordance context.
...
**Primary output:** Validated Semantic Plan under Contracts/SharedContracts.md plus the Planner-owned reasoning explanation.
...
**Approved contract relationships:** Demonstration data from ENG-021 plus Knowledge, Affordance, and Semantic Inventory context. Planner emits its own Explanation Records for ENG-016 aggregation and its Semantic Plan for ENG-014 compilation.
...
This Engine does not own TaskIR compilation/serialization, user approval, execution, simulation communication, or knowledge persistence. It shall not bypass another architectural layer, duplicate another Engine's decision authority, or absorb convenience behavior outside its catalogue responsibility.
...
**Draft state model:** Idle>ValidatingGoal>Planning>ValidatingPlan>Completed|NoPlan|Failed; result>Idle.
...
| Engine purpose/responsibilities | ABP-04 ENG-015 |
