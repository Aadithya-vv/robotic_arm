# ENG-020 — Dashboard Engine Specification
...
Provide the primary user interface and own explicit user approval for execution. This specification refines the catalogue responsibility into an implementation-reviewable behavioral boundary without selecting algorithms, concrete APIs, or concrete dependencies.

## 2. Scope

The Engine accepts its approved architectural inputs, validates them, performs the responsibilities owned by ENG-020, returns explicit outcomes, and exposes sufficient lifecycle and diagnostic information for deterministic coordination and review.
...
- Display execution.
- Capture explicit user approval and issue Approval Tokens bound to exact TaskIR artifacts.

Each responsibility is exclusive to this Engine at the ownership level defined by ABP-04.

## 4. Inputs

**Primary input:** Approved observable state, explanations exposed by ENG-016, execution progress, validated TaskIR presented for approval, and user actions.
...
**Primary output:** Human-readable interface state, validated user interaction events, and Approval Tokens under Contracts/SharedContracts.md.
...
**Approved contract relationships:** ENG-016 provides exposed explanations; ENG-014 provides validated TaskIR for presentation; ENG-020 issues Approval Tokens consumed by ENG-017. Other platform Engines provide observable state through contracts.

These relationships are approved at the architectural ownership level and shall use the repository-level Contracts package. Concrete dependency bindings remain prohibited by Rule 40. Any still-unspecified optionality, lifecycle order, or failure propagation must be resolved without changing the approved direction and ownership.

## 9. Non-Responsibilities

This Engine does not own domain reasoning, explanation aggregation, TaskIR compilation, execution scheduling/control, knowledge mutation, or simulation communication. It shall not bypass another architectural layer, duplicate another Engine's decision authority, or absorb convenience behavior outside its catalogue responsibility.
...
**Draft state model:** Starting>Ready; Ready>Rendering>Ready; Ready>AwaitingInput>Ready; Ready>Stopped; active states may enter Degraded.
...
| Engine purpose/responsibilities | ABP-04 ENG-020 |
...
- Preserve DependencyMap.md and Contracts ownership boundaries; make implementation choices within them and escalate only genuine architectural ambiguity.
