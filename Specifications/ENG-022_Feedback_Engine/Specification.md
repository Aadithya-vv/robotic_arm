# ENG-022 — Feedback Engine Specification
...
Capture user feedback. This specification refines the catalogue responsibility into an implementation-reviewable behavioral boundary without selecting algorithms, concrete APIs, or concrete dependencies.

## 2. Scope

The Engine accepts its approved architectural inputs, validates them, performs the responsibilities owned by ENG-022, returns explicit outcomes, and exposes sufficient lifecycle and diagnostic information for deterministic coordination and review.

## 3. Responsibilities

- Record corrections.
- Record validation.
- Improve semantic knowledge by generating Knowledge Update Requests; never modify Knowledge directly.

Each responsibility is exclusive to this Engine at the ownership level defined by ABP-04.

## 4. Inputs

**Primary input:** User correction/validation tied to traceable system output.
...
**Primary output:** Validated feedback record and Knowledge Update Request under Contracts/SharedContracts.md.
...
**Approved contract relationships:** Dashboard supplies user feedback; ENG-022 emits Knowledge Update Requests to ENG-012; Event Bus and Logging provide platform services. ENG-012 alone validates and applies updates.
...
This Engine does not own Knowledge validation/application, knowledge storage or mutation, semantic reasoning, planning, or UI rendering. It shall not bypass another architectural layer, duplicate another Engine's decision authority, or absorb convenience behavior outside its catalogue responsibility.
...
**Draft state model:** Idle>Capturing>Validating>Recorded; Recorded>GeneratingUpdateRequest>Submitted|Rejected|Failed; terminal>Idle.
...
| Engine purpose/responsibilities | ABP-04 ENG-022 |
