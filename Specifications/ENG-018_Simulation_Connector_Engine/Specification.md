# ENG-018 — Simulation Connector Engine Specification
...
| Engine | ENG-018 — Simulation Connector Engine |
...
Communicate with the robotic arm simulation. This specification refines the catalogue responsibility into an implementation-reviewable behavioral boundary without selecting algorithms, concrete APIs, or concrete dependencies.

## 2. Scope

The Engine accepts its approved architectural inputs, validates them, performs the responsibilities owned by ENG-018, returns explicit outcomes, and exposes sufficient lifecycle and diagnostic information for deterministic coordination and review.
...
**Primary input:** Connection request and execution exchange request.
...
**Primary output:** Connection state, acknowledgement, simulation response, or communication failure.
...
**Contract relationships for engineering:** Execution contract, Configuration, Logging, and independent PyBullet SDK/network boundary.
...
This Engine does not own execution scheduling, TaskIR validation, simulation internals, or robotic control policy. It shall not bypass another architectural layer, duplicate another Engine's decision authority, or absorb convenience behavior outside its catalogue responsibility.
...
**Draft state model:** Disconnected>Connecting>Connected; Connected>Exchanging>Connected|Degraded; Connected>Disconnecting>Disconnected.
...
| Engine purpose/responsibilities | ABP-04 ENG-018 |
