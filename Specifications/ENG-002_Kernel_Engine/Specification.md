# ENG-002 — Kernel Engine Specification
...
Coordinate the overall runtime. This specification refines the catalogue responsibility into an implementation-reviewable behavioral boundary without selecting algorithms, concrete APIs, or concrete dependencies.

## 2. Scope

The Engine accepts its approved architectural inputs, validates them, performs the responsibilities owned by ENG-002, returns explicit outcomes, and exposes sufficient lifecycle and diagnostic information for deterministic coordination and review.
...
**Primary input:** Registered platform services and lifecycle requests.
...
**Primary output:** Coordinated runtime state and lifecycle results.
...
**Contract relationships for engineering:** Registry, Event Bus, Memory, Configuration, and Logging contracts.
...
This Engine does not own Engine discovery, event transport, configuration parsing, or domain reasoning. It shall not bypass another architectural layer, duplicate another Engine's decision authority, or absorb convenience behavior outside its catalogue responsibility.
...
**Draft state model:** Created>Starting>Running>Stopping>Stopped; active states may enter Failed.
...
| Engine purpose/responsibilities | ABP-04 ENG-002 |
